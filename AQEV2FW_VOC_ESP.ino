...
void help_menu(char * arg) {
  ...
  if (arg == 0) {
    ...
    else if (strncmp("co2_blv", arg, 7) == 0) {
      get_help_indent(); Serial.println(F("<sub-command> is one of 'add', 'clear', 'show'"));
      get_help_indent(); Serial.println(F("co2_blv add [temperature] [slope] [intercept]"));
      get_help_indent(); Serial.println(F("    adds a row to the characterization table"));
      get_help_indent(); Serial.println(F("co2_blv show"));
    ...
    switch(token_number++){
      case 0:
        ...
        if(l_sample_interval < 1){
          Serial.print(F("Error: Sampling interval must be greater than 1 [was ")); 
