...
#define SCRATCH_BUFFER_SIZE (768)
...
boolean add_baseline_voltage_characterization(char * arg, uint32_t eeprom_table_base_address){
  ...
  if(!empty_location_found){
    ...
    if(eeprom_table_base_address == EEPROM_CO2_BASELINE_VOLTAGE_TABLE){
      Serial.print(F("co2"));
    }
...
boolean burstSampleADC(int adcChannel, float * result){
  ...
  const float adc_resolution_volts_per_lsb = (5.0f / 1024.0f);
  *result = (adc_resolution_volts_per_lsb * burst_sample_total) / NUM_SAMPLES_PER_BURST;  
...
boolean publishIAQCore(){
  ...
  // equivalent co2
  float co2_moving_average = calculateAverage(&(sample_buffer[CO2_EQUIVALENT_SAMPLE_BUFFER][0]), num_samples);
...
void printCsvDataLine(){
  ...
  if(iaqcore_ready || (sample_buffer_idx > 0)){
    float converted_value = 0.0f, compensated_value = 0.0f;    
    co2_moving_average = calculateAverage(&(sample_buffer[CO2_EQUIVALENT_SAMPLE_BUFFER][0]), num_samples);
    co2_equivalent_compensation(co2_moving_average, &converted_value, &compensated_value);
    co2_equivalent_ppm = compensated_value;        
